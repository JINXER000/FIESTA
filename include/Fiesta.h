...
void Fiesta<DepthMsgType, PoseMsgType>::Visualization(ESDFMap *esdf_map, bool global_vis, const std::string &text)
{
  ...
  std::cout <<"occu cld size is "<<pc.points.size()<<std::endl;
  ...
  for (int idx = part * i; idx < part * (i + 1); idx++)
  {
    ...
    if (tmp_idx != -10000)
    {
      ...
      if (set_occ_[tmp_idx] == tt)
        continue;   // no need to do ray cast on point
      else
        set_occ_[tmp_idx] = tt;
#endif
    }
      ...
      for (int v = parameters_.depth_filter_margin_; v < rows - parameters_.depth_filter_margin_; v++)
      {
        ...
        for (int u = parameters_.depth_filter_margin_; u < cols - parameters_.depth_filter_margin_; u++)
        {
          ...
          if (uu >= 0 && uu < cols && vv >= 0 && vv < rows)
          {
            ...
            if (fabs(last_img.at<uint16_t>((int)vv, (int)uu) / k_depth_scaling_factor - coord.z()) < parameters_.depth_filter_tolerance_)
            {
              cloud_.push_back(point);
            }
...
void Fiesta<DepthMsgType, PoseMsgType>::DepthCallback(const DepthMsgType &depth_map)
{
  ...
  if (!got_depth)
  {
    std::cout << "set t0, t1, t2 " << std::endl;
    t0 = depth_map->header.stamp;
    t1 = t0 + ros::Duration(3);

    got_depth = true;
  }
  cur_stamp = depth_map->header.stamp;
}
...
void Fiesta<DepthMsgType, PoseMsgType>::UpdateEsdfEvent(const ros::TimerEvent & /*event*/)
{
  ...
  if (esdf_map_->CheckUpdate())
  {
    ...
    if(occu_q_empty)
    {
      sdf_rcd->record((float)duration, "t2_sdf");
      occu_q_empty = false;
      printf("occupancy queue is empty!\n");
  ...
  ros::Duration tm1_diff = cur_stamp - t1;
  ros::Duration tm2_diff = ros::Time::now() - t2;
  std::cout << " -------tm1_diff: " << tm1_diff << " tm2_diff is " << tm2_diff << std::endl;
  if (tm1_diff < ros::Duration(0.25) && tm1_diff > ros::Duration(-0.25) && !t1_done) // with t1
  {
    ...
    if (esdf_map_->CheckUpdate())
    {
      ...
      auto end = std::chrono::steady_clock::now();
      auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
      sdf_rcd->record((float)duration, "t1_sdf: ");
      std::cout << "Dense mapping time: "
                << duration
                << " ms" << std::endl;
    }
